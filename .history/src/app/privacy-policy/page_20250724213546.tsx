  ...
  {others.map((init) => {
    ...
    return (
      <Link
        ...
        key={init.slug}
        className="group flex justify-between items-center gap-6 p-6 bg-card/50 transition-colors duration-300 hover:bg-card/70"
      >
        {/* Gamepad icon: most left, vertically centered, no gap */}
        <div className="flex-shrink-0 text-primary flex items-center">
          <Gamepad className="w-8 h-8" />
        </div>

        {/* Initiative icon + text, no gap between Gamepad and this */}
