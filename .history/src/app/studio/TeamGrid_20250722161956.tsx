...
export default function TeamGrid({ leaders, crew }: TeamGridProps) {
  const renderMember = ({ name, position, fullPosition, avatar }: Member, key: string) => {
    ...
    return (
      ...
      >
        <div className="flex flex-row sm:flex-col items-center sm:items-start gap-4 sm:gap-0 mt-0 sm:mt-4">
          {/* Avatar Image */}
      ...
      <div className="max-w-6xl mx-auto px-4 relative z-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {leaders.map((member, i) => (
            ...
            >
              <div className="w-fit">{renderMember(member, member.name + '-leader')}</div>
            </div>
          ))}
        </div>
