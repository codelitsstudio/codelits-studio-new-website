...
export default function TeamGrid({ leaders, crew }: TeamGridProps) {
  const renderMember = ({ name, position, fullPosition, avatar, linkedin }: Member, key: string) => {
    ...
    return (
      ...
      {/* Name and Position with LinkedIn on Position */}
      <div className="mt-4 flex items-center justify-start gap-14">
        <div>
          <p className="font-semibold text-lg text-white transition-colors">
            {name}
          </p>
          <p className="text-sm group-hover:text-primary text-muted-foreground mt-2 flex items-center gap-2">
            {position} |
            {linkedin && (
              <a
                href={linkedin}
                ...
                className="text-muted-foreground mb-1 hover:text-primary transition"
