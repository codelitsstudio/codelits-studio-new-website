...
export default function TeamGrid({ leaders, crew }: TeamGridProps) {
  const renderMember = ({ name, position, fullPosition, avatar }: Member, key: string) => {
    ...
    return (
      ...
      >
       <div className="flex flex-row sm:flex-col items-center gap-4 sm:gap-0 mt-0 sm:mt-4">
  <div
  className={`flex ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'} sm:flex-col items-center gap-4 sm:gap-0 mt-0 sm:mt-4`}
>
  {/* Avatar Image */}
  <div className="w-24 sm:w-32 aspect-square rounded-xl overflow-hidden group shrink-0">
    <Image
      src={avatar}
      alt={name}
      width={128}
  ...
  <div className="flex flex-col items-start text-left">
    <p className="font-semibold text-base sm:text-lg text-white transition-colors">
      {name}
    </p>
...
{leaders.map((member, i) => (
  <div
    ...
    className={`flex ${i % 2 === 0 ? 'justify-start' : 'justify-end'} sm:justify-start`}
  >
    <div className="w-fit">
      {renderMember(member, member.name + '-leader', i)}
    </div>
  </div>
))}
</div>
  ...
  {crew.map((member, i) => (
    <div
      key={member.name + '-crew'}
      className={`sm:col-span-1 ${i % 2 === 0 ? 'text-left' : 'text-right'} sm:text-left`}
    >
