...
export default function Hero() {
 ...
 useEffect(() => {
  const ctx = gsap.context(() => {
    // Set initial styles to hide/offset elements before animation
    gsap.set('.hero-title-char', { y: 100, opacity: 0 });
    gsap.set('.hero-p', { y: 20, opacity: 0 });
    gsap.set('.hero-buttons', { y: 20, opacity: 0 });
    gsap.set('.hero-image', { y: 100, opacity: 0, scale: 0.9 });

    const tl = gsap.timeline({
      defaults: { ease: 'power4.out', duration: 1.2 }
    });

    tl.to('.hero-title-char', { y: 0, opacity: 1, stagger: 0.05, duration: 0.6 })
      .to('.hero-p', { y: 0, opacity: 1, duration: 1 }, '-=0.8')
      .to('.hero-buttons', { y: 0, opacity: 1, duration: 1 }, '-=0.8')
      .to('.hero-image', { y: 0, opacity: 1, scale: 1, duration: 1.5, ease: 'elastic.out(1, 0.75)' }, '-=0.8');
  ...
  <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center px-4">
    ...
    >
      ...
      <p className="mb-4 text-sm md:text-base text-muted-foreground">
        ...
        <br />
        You can visit our old version here  → {' '}
        <a
          href="https://old.codelitsstudio.com"
          target="_blank"
      ...
      <div className="flex justify-center gap-3 mt-5 flex-col sm:flex-row">
        <Button
          className="text-sm px-4 py-2 font-normal text-black dark:text-black"
