...
export function Header() {
  ...
  // Animate loader: from 0 to 100% width, then hide
  const runLoaderAnimation = () => {
    ...
    // Reset to 0 width and visible
    gsap.set(loader, { width: 0, autoAlpha: 1 });

    // Animate width to 100% over 1.5 seconds
    gsap.to(loader, {
      width: "100%",
      duration: 1.5,
      ease: "power1.out",
      onComplete: () => {
        // Fade out loader after full width
        gsap.to(loader, { autoAlpha: 0, duration: 0.3, delay: 0.2 });
      },
    });
  };
  ...
  return (
    <header
      ref={headerRef}
      className="sticky top-0 z-50 w-full p-2 md:p-4 bg-white"
    >
      ...
      <div
        ref={loaderRef}
        className="h-[3px] w-0 bg-primary transition-width duration-300"
...
// ListItem Component with onClick support
