...
export function Header() {
  ...
  useEffect(() => {
    ...
    const showHeader = () => {
      if (!header) return;
      gsap.to(header, { y: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.1)", backdropFilter: "blur(6px)", ease: "power2.out", duration: 0.5, delay: 0.1 });
      isActive = true;
    };
    const hideHeader = () => {
      if (!header) return;
      gsap.to(header, { y: 0, boxShadow: "0 0 0 rgba(0,0,0,0)", backdropFilter: "blur(0px)", ease: "power2.out", duration: 0.5, delay: 0.1 });
      isActive = false;
    };
    const handleScroll = () => {
  ...
  // Loader animation on path change fallback
  useEffect(() => {
    const loader = loaderRef.current;
    if (!loader) return;
    // Start full-width animation on each pathname change
      ...
      {aiItems.map((item) => (
        <ListItem key={item.title} title={item.title} href={item.href}>
          {item.description}
            ...
            <NavigationMenuItem>
              <NavigationMenuLink asChild>
                <Link href="/studio" className="nav-click-anim">
                  Studio
                </Link>
              </NavigationMenuLink>
            </NavigationMenuItem>
