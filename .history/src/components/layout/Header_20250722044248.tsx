...
export function Header() {
  ...
  // Prevent render for admin pages
  ...
  // Loader animation triggered on link clicks
  const runLoaderAnimation = () => {
    ...
    // Make loader visible, small height, zero width to start
    gsap.set(loader, { width: 0, height: 4, autoAlpha: 1 });

    // Animate width to full (100%) over 2.5s
    gsap.to(loader, {
      width: "100%",
      duration: 2.5,
      ease: "power2.out",
      onComplete: () => {
        // Fade out and shrink height back to 0
        gsap.to(loader, {
          autoAlpha: 0,
          height: 0,
          duration: 0.5,
  ...
  return (
    <header ref={headerRef} className="sticky top-0 z-50 w-full p-2 md:p-4 relative">
      <div className="container mx-auto flex h-16 max-w-screen-2xl items-center justify-between px-4">
        ...
        <NavigationMenu className="hidden md:flex gap-4">
          <NavigationMenuList className="flex gap-4 text-sm font-medium items-center">
            ...
            <NavigationMenuItem>
              <NavigationMenuTrigger>
                <span className="cursor-pointer nav-click-anim" onClick={runLoaderAnimation}>
                  Work
                </span>
              </NavigationMenuTrigger>
              <NavigationMenuContent>
      ...
      <div
        ref={loaderRef}
        className="absolute bottom-0 left-0 bg-primary origin-left transition-[height,opacity]"
        style={{ width: 0, height: 0, willChange: "width, height, opacity" }}
      />
    </header>
  );
}
