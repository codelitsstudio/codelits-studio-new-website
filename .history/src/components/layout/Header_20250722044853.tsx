...
const serviceItems = [
  ...
  },
  {
    ...
    description:
      "Engaging native and cross-platform mobile applications.",
  },
  {
...
export function Header() {
  ...
  // ---- NEW: real loader animation tied to route events ----
  useEffect(() => {
    ...
    const finish = () => {
      // complete to 100% then fade/slide away
      gsap.killTweensOf(loader);
      gsap.to(loader, {
        width: "100%",
        duration: 0.3,
  ...
  // Animate loader: width from 0 to 100% then fade out
const runLoaderAnimation = () => {
  if (!loaderRef.current) return;
  const loader = loaderRef.current;

  gsap.killTweensOf(loader);
  gsap.set(loader, { height: 0, autoAlpha: 1 });

  gsap.to(loader, {
    height: 4, // height grows downwards to 4px
    duration: 1.2,
    ease: "power1.out",
    onComplete: () => {
      gsap.to(loader, { autoAlpha: 0, height: 0, duration: 0.3, delay: 0.2 });
    },
  });
};
...
<header
  ref={headerRef}
  className="sticky top-0 z-50 w-full p-2 md:p-4 relative" // <-- added relative here
>
      <div className="container mx-auto flex h-16 max-w-screen-2xl items-center justify-between px-4">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2" onClick={runLoaderAnimation}>
          <Image
            src="/logos/logo.png"
  ...
  <div
    ref={loaderRef}
    className="w-full h-0 bg-primary origin-top transition-all absolute bottom-0 left-0"
